package com.superfunapp.pointvendor;
...
public class SignupActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (SharedPrefrnceSuperFun.getSharedPrefData(signUp, "vendor_verified") != null) {
			if (SharedPrefrnceSuperFun.getSharedPrefData(signUp,
					"vendor_verified").equals("true")) {
	...
	public void onClick(View v) {
		...
		case R.id.submitBtn:
			...
			SharedPrefrnceSuperFun.setDataInSharedPrefrence(this, Constants.BRANCH_ID, branchCode);
			...
			if(uuidStr.length() == 0 || passwordStr.length() == 0 || simNumberStr.length() == 0 || branchCode.length() == 0){
				...
				if (NetworkAvailablity.chkStatus(this))
					new RegisterVendor().execute(uuidStr, simNumberStr,
							branchCode, passwordStr);
				else
					alertDialogManager.showAlertDialog(this, "Network Error", getResources().getString(R.string.network_connection));
			}
			
			
			/*if (uuidStr.length() != 0 && passwordStr.length() != 0
					&& simNumberStr.length() != 0 && branchCode.length() != 0) {

				SharedPrefrnceSuperFun.setDataInSharedPrefrence(signUp,
						Constants.BRANCH_ID, branchCode);
	...
	public class RegisterVendor extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			nameValuePairs
					.add(new BasicNameValuePair("station_type", "Points"));

			String json = serviceHandler.makeServiceCall(
					WebServiceDetails.WS_URL + WebServiceDetails.REGISTER,
				...
				try {
					...
					if (jsonObject != null) {
						...
						SharedPrefrnceSuperFun.setDataInSharedPrefrence(
								SignupActivity.this, Constants.BRANCH_ADDRESS,
								data.optString("address"));
						SharedPrefrnceSuperFun.setDataInSharedPrefrence(
								SignupActivity.this, Constants.BRANCH_NAME,
								data.optString("branchname"));
						SharedPrefrnceSuperFun.setDataInSharedPrefrence(
								SignupActivity.this, Constants.COMPANY_NAME,
								data.optString("companyname"));
						SharedPrefrnceSuperFun.setDataInSharedPrefrence(
								SignupActivity.this, Constants.COMPANY_ID,
								data.optString("companyid"));
						Intent intent = new Intent(signUp, HomeScreen.class);
						startActivity(intent);
